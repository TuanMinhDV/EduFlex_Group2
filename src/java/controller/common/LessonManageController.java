...
public class LessonManageController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet LessonManageController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LessonManageController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (type.equalsIgnoreCase("add")) {
            ...
            Lesson lessNew = new Lesson(nameLesson, video, document, "abccc", chapID);
...
//            request.getRequestDispatcher("test.jsp").forward(request, response);
    ...
    public void loadDBwithChapterID(int chap_Id, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("ManageLessonsByIns.jsp").forward(request, response);
    }
